...
- **Voice is the only input**: there is no text mode, so a session can't start without a mic. `ui/src/mic-check.ts` pre-flights both start paths — a silent `probeMic()` paints the setup notice and disables Begin when it's certain, and `acquireMicOnce()` runs first inside the Begin handler (before any await, so the permission prompt keeps the click's user gesture). The same voice-only rule gives `isMuteCommand` (`mute-command.ts`) its job: a bare "mute" outranks every dispatch route, since it's the one thing you say when you want the app to stop hearing you. Deliberately strict — losing the mic mid-sentence beats reaching for the button — so the whole utterance must be the command. Unmuting is button-only. Hosted sessions also take natural-language commands (`voice-command.ts`, asks in `voice-command-specs.ts`): end session (plain follows `saveSessionLogs`; "end without saving" and "end and save" override it for that sit), speak slower/faster, set/extend/cancel the timer, time check, repeat the last line, reply sooner / wait longer (one rung on `PAUSE_LADDER`, applied live through the optional `SttEngine.setPauseWindow` and saved to `silenceBaseMs`/`silenceMaxMs`), and natural-phrased mute (the bare word still takes the judge-free `isMuteCommand` path first, so it works on every provider). Short utterances (`mightBeCommand`) go to Jev as one request with an atomic yes/no ask per command, right after the mute check and ahead of `routeUtterance`, so they work inside a hold without ending it (`restoreHoldAfterNotice`). Jev says *that* a timer was asked for; `parseTimerRequest` pulls the minutes. Each command speaks a canned acknowledgment (`COMMAND_LINES`), and ending is only ever a bid: the app asks, and `classifyEndConfirm` judges the reply inside a 45s window. No judge (BYOK, local, outage) means no commands, only the bare `mute`. Discovery is `ui/src/voice-command-hints.ts`, all on the status line rather than toasts: the first few hosted sessions read `Listening… or ask "what can I say?"` (the `help` command), using the speed slider, timer picker or mute button by hand shows a one-time `You can just say "..."`, and the session info panel has a Voice commands row that opens the list.
