...
- **Pacing state machine**: IDLE → LISTENING → PROCESSING → RESPONDING → SILENT_HOLD; a check-in fires after a silence interval and the timer resets. Check-in content is a setting: a canned (non-LLM) phrase, or "smart" (`smart-checkin.ts`) — the LLM gets the session plus a bracketed silence-event turn and offers one short line in context or replies `[PASS]` to keep quiet; unusable replies fall back to the canned pool.
