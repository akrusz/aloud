...
- **ModeSpec registry** (`modes.ts`): meditation modes are data, not forks - base prompt, which user dimensions compose, opener/check-in pools, and (for staged modes like felt sense, `felt-sense.ts`) an ordered phase arc. The whole arc sits in the (frozen) system prompt and the active phase travels as a stage-note entry in the session log; the LLM moves phases with `[NEXT]`/`[BACK]`, clamped + persisted by `StagedModeController` and `SessionState.modePhase`.
- **Append-only session log** (`SessionState.exchanges`): exactly what the model was sent, so each request extends the last and reads it from prompt cache. The system prompt is frozen for the sit; anything mid-sit is appended, never edited - the opener instruction, spoken check-in/timer events, and phase/setting notes as control entries (`Exchange.kind`), notes as `role: 'system'` (a mid-conversation system message where the Anthropic model takes one, `<system-reminder>` user text elsewhere). Views filter with `spokenExchanges`. `tests/cache-prefix.test.ts` pins the invariant.
