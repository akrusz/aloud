...
export function llmRoutes(deps: Deps): Hono<{ Variables: AuthVars }> {
    ...
    app.post('/complete', requireAuth(deps), async (c) => {
        ...
        if (!provider || !VALID_PROVIDERS.has(provider) || !model || !Array.isArray(messages)) {
            return errorJson(c, 'bad_request', 'provider, model, messages required');
        }
        if (!isModelAllowed(provider, model)) {
            return errorJson(c, 'model_not_allowed', `model not available on aloud cloud: ${provider}/${model}`);
        }
        ...
        if (!pass) {
            ...
            if (balance <= 0) {
                incident('insufficient_credits', 'llm: balance 0');
                return errorJson(c, 'insufficient_credits', 'out of credits');
            }
            const promptTexts = [body.system ?? '', ...messages.map((m) => m.content ?? '')];
            const holdAmount = holdAgainstBalance(holdForTurn(provider, model, promptTexts, maxTokens), balance);
            try {
                holdId = await deps.ledger.placeHold(account.id, holdAmount, `turn:${provider}:${model}`);
            } catch (err) {
                if (!(err instanceof InsufficientCreditsError)) throw err;
                incident('insufficient_credits', `llm: hold ${holdAmount.toFixed(2)} > balance ${balance.toFixed(2)}`);
                return errorJson(c, 'insufficient_credits', 'out of credits');
            }
        }

        const fwd = { provider, model, maxTokens, ...(body.system ? { system: body.system } : {}) };

        /** Bill a finished turn: settle the hold to the ACTUAL metered cost
         *  (releasing the remainder), record usage, flag a blank reply, and
         *  build the result. Usage records the metered credits even when a pass
         *  covers the turn, so per-retreat spend and the daily-cap sum stay
         *  honest; the response charges a covered turn nothing. */
        let settled = false;
        const settle = async (
            text: string,
            finishReason: string | null,
        ...
        if (body.stream) {
            return streamSSE(c, async (sse) => {
                try {
                    ...
                    let sawText = false;
                    for await (const chunk of deps.forwarder.stream(messages, fwd)) {
                        if (!chunk.done) {
                    ...
                    log.error('stream forward failed', { err: String(err), provider });
                    incident('llm_error', `stream: ${String(err)}`);
        ...
        try {
            ...
            return errorJson(c, 'provider_error', 'upstream provider error');
        }
    });
