 ...
 * adds the clouds here; decline returns them to the buyer (no refund, no risk —
 * the payment already cleared). Reuses the `.voice-modal-*` chrome.
 */
...
function showGiftModal(gifts: GiftView[]): void {
    ...
    overlay.innerHTML = `
        <div class="voice-modal gift-modal" role="dialog" aria-modal="true" aria-label="Gifted clouds">
            ...
            <p class="provider-hint gift-modal-subtitle">Accept to add them to your balance, or decline to return them to the sender.</p>
    ...
    for (const gift of gifts) {
        ...
        row.querySelector('.gift-decline')?.addEventListener('click', () =>
            resolve(declineGift, () => {
                /* returned to sender — no toast needed */
            })
        );
